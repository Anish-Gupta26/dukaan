...
const Body = () => {
  return (
    ...
    <div className="main-body">
        ...
        <br/><br/>
        <h4 className='search-title'>Word for your slogan</h4>
        ...
        <div className="row search-result">
            {data.map((item)=>{
                return(
                    <>
                    <div className="search-result-card col-sm-6 card mt-3 mp-2" data-aos="fade-zoom-in"  data-aos-duration="1000">{item}</div>
                    </>
                )
            })}
        </div>
        <hr className='my-3'/>
        <button type="button" className="btn btn-outline-primary next-button">Next &gt;</button>
    </div>
  )
}
